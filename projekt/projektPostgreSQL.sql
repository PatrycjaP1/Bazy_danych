...
CREATE TABLE ZdGeoTabela AS
    (SELECT * FROM GeoPietro NATURAL JOIN GeoEpoka NATURAL JOIN GeoOkres NATURAL JOIN GeoEra NATURAL JOIN GeoEon);
SELECT * FROM ZdGeoTabela;
--1
SELECT COUNT(*) FROM Milion
INNER JOIN ZdGeoTabela ON (mod(Milion.liczba,68)=(ZdGeoTabela.id_pietro));
...
(SELECT id_pietro FROM ZdGeoTabela WHERE mod(Milion.liczba,68)=(id_pietro));
